...
export class Game {
     ...
     * @param {Levels[]} levels Tous les niveaux de jeu
    ...
    #handleGamePaused() {
        if (!this.#pauseMessageOnDisplay) {
            this.#canvasElement.drawText(PAUSE_MESSAGE, FONT_MESSAGE, 0.4,0.5);
    ...
    #handleEndOfGame() {
        if (!this.#winMessageOnDisplay) {
            ...
            for (let i=0; i<END_OF_GAME_MESSAGE.length; i++) {
                this.#canvasElement.drawText(END_OF_GAME_MESSAGE[i], FONT_MESSAGE, 0.4, 0.3+0.2*i);
            }
            setTimeout(() => this.#win = null, 3000);
        }
    }
